...
if (PLAY_STATES.length < CURRENT_DAY) {
    ...
    if (actualHeardles.length <= expectedHeardles.length && actualHeardles.some((h, i) => h !== expectedHeardles[i])) {
        ...
        if (expectedList !== undefined && actualList.some((h, i) => h !== expectedList[i])) {
