...
$modalsChildren.forEach(($element) =>
	$element.on("click", (e) => {
		e.stopPropagation();
	})
);
...
$openStats.on("click", () => {
	...
	if (maxBar > 0) {
		$modalStatsBars.forEach(($element, index) => {
			...
			if (CURRENT_PLAY_STATE.finished && CURRENT_PLAY_STATE.failed === index) {
				$element
					.removeClass("bg-custom-mg")
					.addClass("bg-custom-positive")
					.removeClass("border-custom-mg")
					.addClass("border-custom-positive");
			}
		});
	}
	...
	if (CURRENT_PLAY_STATE.finished) {
		...
		$modalStatsCounters[CURRENT_PLAY_STATE.failed]
			...
			.removeClass("text-custom-fg")
			.addClass("text-custom-positive");
	...
	$modalStatsWinRate.text(
		STATISTICS.viewed === 0 ? "-" : ((STATISTICS.cleared / STATISTICS.viewed) * 100).toFixed(1) + "%"
	);
