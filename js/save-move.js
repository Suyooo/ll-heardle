...
if (window.location.hash === "#export-save") {
	...
	const saveDataTextarea = $("<textarea>")
		.addClass("mt-6 mb-6 h-32")
		.val(
			`{"playStates":` +
				localStorage.getItem("play_states") +
				`,"statistics":` +
	...
	window.history.replaceState(null, "", "/");
	const movingSaveData = prompt(
		"Please paste your save data here to load it.\n\n" +
	...
	if (movingSaveData) {
		...
		try {
			...
			alert(
				'There was a problem moving your save data. If the error contains something like "JSON", make ' +
