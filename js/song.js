...
const CURRENT_DAY =
	Math.round(
		(Date.UTC(NOW_DATE.getFullYear(), NOW_DATE.getMonth(), NOW_DATE.getDate()) -
			Date.UTC(FIRST_DAY_DATE.getFullYear(), FIRST_DAY_DATE.getMonth(), FIRST_DAY_DATE.getDate())) /
			MS_PER_DAY
	) + 1;
...
function getHeardleIdForDay(day) {
	...
	prngSeed(day);
	const filteredSongPool = SONGPOOL.map((song, id) => ({ id, ...song })).filter((s) => day >= s.startOnDay);
